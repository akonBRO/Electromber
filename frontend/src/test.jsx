...
const techniciansData = {
    Hero: [
        { id: 1, name: 'Robiul Islam', rating: 4.5, jobs: 89, eta: 45, serviceFee: 250, avatar: 'https://placehold.co/48x48/7c3aed/white?text=R' },
        { id: 4, name: 'Suman Barua', rating: 4.6, jobs: 72, eta: 50, serviceFee: 250, avatar: 'https://placehold.co/48x48/f59e0b/white?text=S' }
    ],
    'Hero Plus': [
        { id: 2, name: 'Jamil Ahmed', rating: 4.8, jobs: 154, eta: 30, serviceFee: 500, avatar: 'https://placehold.co/48x48/db2777/white?text=J' },
        { id: 5, name: 'Fahim Chowdhury', rating: 4.7, jobs: 130, eta: 35, serviceFee: 500, avatar: 'https://placehold.co/48x48/1d4ed8/white?text=F' }
    ],
    Superhero: [
...
const availableAreas = ['Uttara', 'Badda', 'Malibagh'];
const allAreas = ['Uttara', 'Malibagh', 'Badda', 'Gulshan', 'Dhanmondi', 'Mirpur'];
const propertyTypes = [
...
const BookServicePage = () => {
    ...
    useEffect(() => {
        const newBasePrice = bookingDetails.problem?.price || [0,0];
        setBasePrice(newBasePrice);
        
        const newServiceFee = bookingDetails.technician?.serviceFee || 0;
        setServiceFee(newServiceFee);
        
        // Set transport fee to 50 if technician is selected and area is available
        const newTransportFee = (bookingDetails.technician && availableAreas.includes(bookingDetails.location.area)) ? 50 : 0;
        setTransportFee(newTransportFee);
        
        // Calculate total price
        const minTotal = newBasePrice[0] + newServiceFee + newTransportFee;
        const maxTotal = newBasePrice[1] + newServiceFee + newTransportFee;
        setTotalPrice([minTotal, maxTotal]);
    }, [bookingDetails.problem, bookingDetails.technician, bookingDetails.location.area]);
    ...
    const isStepComplete = (stepNum) => {
        switch (stepNum) {
            ...
            case 3: return !!bookingDetails.location.area && availableAreas.includes(bookingDetails.location.area) && !!bookingDetails.location.address;
                            ...
                            <div>
                                {availableAreas.includes(bookingDetails.location.area) ? (
                                    ...
                                    <div className="p-6 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-300 rounded-lg border border-red-200 dark:border-red-900 text-center">
                                        ...
                                        <p className="text-sm mt-1">We currently only serve Uttara, Badda, and Malibagh. We're expanding soon!</p>
                                    </div>
                                ) : (
            ...
            case 4: // Choose Technician
                ...
                return (
                    <SectionWrapper 
                        ...
                        subtitle="Choose from our verified professionals"
                        stepNumber="4"
                    >
                        {availableAreas.includes(bookingDetails.location.area) ? (
                            ...
                            <div className="p-8 text-center">
                                ...
                                <h3 className="text-xl font-bold text-gray-700 dark:text-gray-300 mb-2">No Technicians Available</h3>
                                <p className="text-gray-500 dark:text-gray-400">
                                    We currently don't have technicians available in {bookingDetails.location.area}. 
                                    Please select a different area or check back later.
                                </p>
                                <button 
                                    onClick={() => setStep(3)} 
                                    className="mt-6 px-6 py-2 bg-gradient-to-r from-orange-500 to-amber-500 text-white rounded-lg font-medium"
                                >
                                    Change Location
                                </button>
                            </div>
